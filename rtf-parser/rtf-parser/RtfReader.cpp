...
bool RtfReader::RtfStream2HtmlString(string &outstring, istream &instream)
{
	...
	try
	{
		...
		utils::stream_logger log(dummy/*cerr*/, true, true, true);
		utils::stream_source src(instream);
		std::stringstream stream;
		RtfReader::html proc(stream);
...
bool RtfReader::RtfString2HtmlString(string &outstring, const string &instring)
{
	std::stringstream instream(instring);
	return RtfStream2HtmlString(outstring, instream);
}
...
bool RtfReader::RtfFile2HtmlString(string &outstring, const char * filename)
{
	ifstream file(filename);
	if (file.is_open())
	{
		bool ret = RtfStream2HtmlString(outstring, file);
		file.close();
		return ret;	
	}
	return false;
